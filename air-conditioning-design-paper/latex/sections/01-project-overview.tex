...
本课程设计以某实际办公楼工程图纸（AutoCAD 2024，天正暖通格式）为建筑原型。经DWG到DXF的几何转换与提取后，在EnergyPlus 23.2平台上构建了逐时建筑能耗仿真模型\cite{crawley2001energyplus,energyplus232docs}。该办公楼总服务建筑面积约\SI{1367}{\meter^2}，共三层，均为地上建筑。建筑占地面积\SI{32.0}{\meter} $\times$ \SI{14.24}{\meter}，建筑总高度\SI{11.4}{\meter}。首层层高\SI{4.2}{\meter}，二、三层层高均为\SI{3.6}{\meter}。建筑朝向为正南北向，南向主立面宽度\SI{32.0}{\meter}。首层建筑面积约\SI{455.7}{\meter^2}，二、三层建筑面积均为\SI{455.7}{\meter^2}。
...
本课程设计将每层建筑平面划分为五个热工分区（thermal zones）：核心区（C）位于平面中央，不直接接触外墙，属于内区；东南西北四个周边区（S、N、W、E）各沿外墙方向布置，直接受室外气象条件影响。三层共计15个热工分区，每个分区独立进行负荷计算与空调末端选型。
...
三层（ZF3） & 174.6 & 115.2 & 115.2 & 25.3 & 25.3 \\
\midrule
合计 & 523.8 & 345.6 & 345.6 & 75.9 & 75.9 \\
\bottomrule
\end{tabular}
\end{table}

总空调服务建筑面积为\SI{1367}{\meter^2}。其中，核心区占总面积的约38.3\%，南、北周边区各占约25.3\%，东、西周边区各占约5.6\%。核心区由于四面均与空调相邻区域接触，其围护结构负荷主要来自屋面和楼板，受室外气温波动的直接影响最小；周边区则直接承受对应方向外墙和外窗的传热与日射负荷，其中南、北区因沿32 m主立面布置，面积和窗墙比较大，负荷占总负荷的主要部分。
...
每层5个空调分区各设置一台VRF末端设备（变制冷剂流量末端），三层共计15台末端。系统采用一台公用的VRF热泵室外机，室外机设计制冷/制热容量由自动选型确定。
...
两类系统方案的空调服务分区数量相同（均为15个）、设计新风量一致（约\SI{1.175}{\meter^3\per\second}，即\SI{4228}{\meter^3\per\hour}）、围护结构和室内设计参数完全相同。因此，本文的比较实质上是在“相同建筑载体、相同负荷边界”下对两类系统拓扑结构及运行性能的横向分析。
