...
为保证多城市比较的公平性，本文以同一共享办公建筑为基础构建母版建筑，并在此基础上派生出五个城市分支\cite{deru2011reference,field2010reference}。建筑原型取自实际工程图纸，共3层，占地\SI{32.0}{\meter} $\times$ \SI{14.24}{\meter}，空调服务建筑面积\SI{1367}{\meter^2}。每层划分为5个热工分区（核心区及东、南、西、北四个周边区），总共形成15个热工分区和15个工况，即5个理想负荷基线工况、5个VRF+DOAS工况和5个FCU+DOAS工况。
...
\caption{基于共享办公建筑母版提取的建筑模型表达，包括各层平面、屋顶平面、四向立面和纵向剖面}
...
在同一城市分支上，本文进一步建立两类实际空调系统。其一为VRF+DOAS方案，由集中设置的VRF室外机、各空调分区的末端设备以及带热回收的新风系统组成\cite{energyplus232docs}。其二为FCU+DOAS方案，由四管制风机盘管、冷冻水系统、热水系统以及独立新风系统组成。两类系统服务的空调分区数量相同，均为15个，设计新风量也保持一致，取\SI{1.175}{\meter^3\per\second}（\SI{4228}{\meter^3\per\hour}）。因此，本文的比较边界可视为”相同建筑服务对象下的不同系统组织形式比较”。
