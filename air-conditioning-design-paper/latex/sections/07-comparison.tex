...
\midrule
沈阳 & 严寒 & 10.00 & 7.31 & 8814 & 203489 & 155.31 \\
天津 & 寒冷 & 17.05 & 12.47 & 13486 & 169458 & 133.83 \\
成都 & 夏热冬冷 & 13.00 & 9.51 & 12437 & 92799 & 76.98 \\
重庆 & 夏热冬冷 & 13.97 & 10.22 & 19114 & 87752 & 78.18 \\
深圳 & 夏热冬暖 & 15.74 & 11.51 & 27080 & 86479 & 83.07 \\
\bottomrule
\end{tabular}
\end{table}
...
年总单位面积负荷方面，沈阳最高（\SI{155.31}{\kilo\watt\hour\per\square\meter}），天津次之（\SI{133.83}{\kilo\watt\hour\per\square\meter}），成都最低（\SI{76.98}{\kilo\watt\hour\per\square\meter}）。成都和重庆两座夏热冬冷城市的总负荷强度十分接近（约\SI{78}{\kilo\watt\hour\per\square\meter}），但深圳（\SI{83.07}{\kilo\watt\hour\per\square\meter}）较之高出约8\%，反映了深圳终年维持室内设计温度所需的综合制冷总时数更长。
...
\begin{table}[tbp]
\centering
\caption{五城市两类系统比较结果汇总（各系统均设15分区/末端，新风量1.175\,m$^3$/s）}
...
\midrule
沈阳 & VRF+DOAS & 61.05 & 61.05 & 60.25 & --- \\
 & FCU+DOAS & 88.84 & 82.84 & 4.89 & 93.33 \\
\midrule
天津 & VRF+DOAS & 71.92 & 71.92 & 60.20 & --- \\
 & FCU+DOAS & 115.71 & 67.26 & 4.81 & 77.03 \\
\midrule
成都 & VRF+DOAS & 76.63 & 76.63 & 48.85 & --- \\
 & FCU+DOAS & 92.24 & 69.75 & 4.64 & 74.81 \\
\midrule
重庆 & VRF+DOAS & 76.59 & 76.59 & 48.06 & --- \\
 & FCU+DOAS & 145.59 & 54.24 & 5.10 & 74.96 \\
\midrule
深圳 & VRF+DOAS & 88.51 & 88.51 & 56.20 & --- \\
 & FCU+DOAS & 162.73 & 55.36 & 5.67 & 45.10 \\
\bottomrule
\end{tabular}
\end{table}
...
（1）VRF方案的设计冷量由北向南递增，从沈阳\SI{61.05}{\kilo\watt}到深圳\SI{88.51}{\kilo\watt}，%
极差为\SI{27.46}{\kilo\watt}。各城市VRF冷热共用同一台热泵室外机，设计冷量与设计热量相等，因此该方案在满足制冷需求的同时也自动提供了等额的供热容量。

（2）FCU方案中，冷水机组设计冷量随制冷需求从沈阳\SI{88.84}{\kilo\watt}增加至深圳\SI{162.73}{\kilo\watt}，各城市间的差异远大于VRF方案。FCU冷水机组设计冷量普遍高于同城市的VRF设计冷量，差异幅度从\SI{15.61}{\kilo\watt}（成都）至\SI{74.22}{\kilo\watt}（深圳）不等。这反映出集中冷源在自动选型时倾向于保留更大的设计余量，且该余量在不同城市间表现出明显差异。

（3）FCU方案中锅炉设计热量在沈阳最高（\SI{82.84}{\kilo\watt}），其次为成都（\SI{69.75}{\kilo\watt}）、天津（\SI{67.26}{\kilo\watt}）、深圳（\SI{55.36}{\kilo\watt}）和重庆（\SI{54.24}{\kilo\watt}），与年热负荷的地理分布高度吻合。沈阳FCU的锅炉设计热量为重庆的1.53倍，反映了严寒地区与夏热冬冷地区在供热容量配置上的合理差异。
...
VRF+DOAS方案的核心优势在于系统的简洁性与灵活调节能力：一台室外机通过制冷剂管路连接15台室内末端，无需建设集中的冷冻水和热水系统，设备占用空间小，系统环路控制相对简单\cite{liu2010vrf_gshp,kanisanchez2017vrf_whole_building}。然而，对该方案而言，其全年运行的经济性和能效高度依赖于室外气象条件：在深圳这样以制冷为主的城市，VRF以较高能效比（EER）运行，年电耗强度降至\SI{56.20}{\kilo\watt\hour\per\square\meter}；在沈阳这样冬季严寒的城市，VRF热泵在低温工况下性能系数（COP）下降明显\cite{aynur2010vrf_desiccant}，全年电耗显著上升。
...
从室内外机容量配比的角度来看（参照参考课程设计关于VRF机组组合率应在50\%--130\%范围内的分析逻辑），本文五个城市的VRF方案中，一台室外机连接15台室内末端，室外机设计冷量范围\SI{61.05}{\kilo\watt}至\SI{88.51}{\kilo\watt}，各分区的末端容量合计约为室外机容量的100\%--130\%，基本遵循了机组组合率的推荐范围。各城市VRF配比率基本一致，说明在相同的建筑分区和负荷分布下，VRF系统在不同气候分区间保持了较为稳定的容量匹配关系。

FCU+DOAS方案中，冷水机组设计冷量与锅炉设计热量的比值随气候冷暖特征变化呈现显著梯度：沈阳1.07:1、成都1.32:1、天津1.72:1、重庆2.68:1、深圳2.94:1。这一比值本身即可作为气候分区的一个定量化标识——比值接近1.0表明该城市供冷与供暖高峰负荷并重（严寒地区），比值大于2.5则表明以绝对制冷需求为主导（夏热冬暖及湿热的夏热冬冷地区）。
...
（3）重庆作为夏热冬冷地区中高温高湿特征最为突出的城市，其FCU冷水机组设计冷量（\SI{145.59}{\kilo\watt}）为五城市中次高（仅次于深圳\SI{162.73}{\kilo\watt}），而VRF设计冷量为\SI{76.59}{\kilo\watt}。重庆FCU冷水机组选型容量显著偏高的现象提示，该城市在采用集中冷源方案时，自动选型算法可能因夏季极端设计日的湿球温度较高而显著放大了设计冷量需求。因此，在重庆等高温高湿城市中，FCU方案在设计阶段应特别关注冷水机组选型的合理性和备用容量设定，避免设计冷量过度安全。
