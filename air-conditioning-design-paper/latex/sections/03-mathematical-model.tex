...
虽然 EnergyPlus 在底层采用的是更为复杂的数值有限差分传热方法（Finite Difference）与全空气房间热平衡法（Heat Balance Method），但上述传统 CLTD/CLF 设计法中关于各热源成分在全天内的物理积蓄与逐时释放关系（通过冷负荷系数 CLF 调节），仍是理解仿真负荷波动特性的理论和实际方法学基础。
