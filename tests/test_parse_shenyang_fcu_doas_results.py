...
def test_build_fcu_doas_summary_returns_shenyang_case_metadata(tmp_path: Path) -> None:
    ...
    summary = build_fcu_doas_summary(
        "shenyang", tmp_path, idf_path=idf_path, floor_area_m2=100.0
    )

    assert summary["case_id"] == "shenyang__fcu_doas"
    assert summary["city"] == "shenyang"
