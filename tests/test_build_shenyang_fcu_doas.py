# Ref: docs/spec/task.md (Task-ID: IMPL-MULTICITY-FCU-HS-001)
...
def test_build_fcu_doas_case_creates_shenyang_case_file(tmp_path: Path) -> None:
    case_path = build_fcu_doas_case("shenyang", output_root=tmp_path)

    assert case_path.name == "shenyang__fcu_doas.idf"
    assert case_path.exists()
    assert (tmp_path / "medium_office_shenyang.idf").exists()

    case_text = case_path.read_text(encoding="utf-8")
    assert "Shenyang_Liaoning_CHN Design_Conditions" in case_text
