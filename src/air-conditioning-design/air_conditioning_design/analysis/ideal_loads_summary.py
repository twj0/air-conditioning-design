    ...
    *,
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if not cooling_indexes or not heating_indexes:
            raise ValueError(
                "Could not find ideal loads cooling/heating rate columns in eplusout.csv"
            )
...
def write_ideal_loads_summary(
    city_id: str,
    results_dir: Path,
    output_path: Path | None = None,
) -> Path:
    ensure_directories()
    summary = build_ideal_loads_summary(city_id, results_dir)
    target = output_path or summary_path_for_case(build_case_id(city_id, "ideal_loads"))
