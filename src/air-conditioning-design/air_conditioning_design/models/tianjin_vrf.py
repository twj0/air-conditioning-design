...
def _build_vrf_condenser(
    donor_objects: list[IdfObject], master_zone: str, zone_names: list[str]
) -> IdfObject:
    condenser = _template_object(
        donor_objects, "AirConditioner:VariableRefrigerantFlow", "VRF Heat Pump"
    )
    condenser.fields[0] = "Tianjin VRF Heat Pump"
    # The Miami donor's -5 C cooling cutoff is too warm for Tianjin core-zone cooling.
...
def build_tianjin_vrf_case(output_root: Path | None = None) -> Path:
    ensure_directories()
    manifest = load_tianjin_manifest()
    ...
    filtered = replace_object(
        ...
        IdfObject(
            "Building",
            [
                "Tianjin Medium Office VRF Pilot",
                "0.0000",
                "City",
                "0.0400",
                "0.2000",
    ...
    vrf_objects = [
        _build_vrf_condenser(donor_objects, master_zone, zone_names),
    ...
    case_path = (
        (output_root / "tianjin__vrf.idf") if output_root else TIANJIN_VRF_PATH
    )
